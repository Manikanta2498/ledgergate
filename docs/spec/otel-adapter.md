...
an iterator over concatenated JSON built on `json.JSONDecoder.raw_decode` with the same
hooks as `loads` and `require_ijson` applied per document, so each document is decoded
separately under the node and depth bounds; the byte bound applies to the file. `[d]` in a report is the document ordinal in either framing (in concatenated framing every
document must be an object; a later array is a shape fault located `[d]`), and the documents'
spans are concatenated in file order. The whole file is decoded before any examination: a decode failure in any document, or trailing non-whitespace after the last, is exit `2` for the file, naming the ordinal in concatenated framing and the decoder's position in array framing (one text, one decode) (a truncated last line from a crashed exporter is therefore
...
present and does not end in `/1.37.0`; two such scopes with differing *present* URLs are a fault, and a scope without a URL beside one with a URL takes the present one. Known consequence: an instrumentation that emits the 1.37 attribute set under a scope URL it was written against earlier (the convention is in development status and libraries lag) is refused wholesale, although the read set would have checked every attribute; that refusal is deliberate, since a URL that names another version is the producer's own statement, and an operator who knows better strips the URL. The URL recorded in `metadata` is producer-controlled: a URL containing a line break or a control character is a located fault whose report names the location only and does not echo the URL (a URL has neither), and the same grammar (one line by the identifier grammar's line-break set, no Unicode `Cc` character) applies to every producer string that reaches `metadata` (resource URLs, and the names, versions and URLs of scopes that contain GenAI spans; a scope with none reaches nothing and is not held to it), and one over 1,024 characters is a fault whose report names the location and a fixed message, echoing nothing (the bound applies to `metadata` without truncation, like every metadata value, and to the *joined* values `otel.scope` and `otel.resource_schema_urls` after joining, a fault naming the key).
...
`absent`) and `otel.scope` (the GenAI scopes' `name version` pairs, `absent` for an omitted member, sorted and joined with `;`; a `;` inside a value is not escaped, since nothing parses these joins). Every `metadata` value is a string of at most 1,024 characters (v1's `StringMap`);
...
does not guess at attributes it was not written for. Spans without `gen_ai.operation.name`, or with one this document does not map (`embeddings`, `create_agent`, ...), produce no events; they are counted in `otel.spans` and take part in the parent check. A run in which the adapter produced **zero events** (no inference span, only error-status ones, or only parts this document does not map) is a completeness fault, a row in the table below, not an empty trace: the adapter observed no conversation and must say so, rather than emit a document that looks like one.
...
1. **Messages.** Inference spans are processed in order of `startTimeUnixNano`, then `endTimeUnixNano` (a span that saw another's output ended no earlier than it, a fact about the run), then position in the file (the residual, equal start *and* end, is a stated consequence: under such a clock the prefix verdict can turn on export order). The adapter keeps the *emitted conversation*: a sequence of
   ...
   becomes a `message` (role `assistant`) at the span's end time and is appended. An inference span whose status is error (`status.code` 2) produces no `message` or `tool_call` events and is skipped by this step entirely (it may still yield a `tool_result` as a result source, step 3), prefix check included, while keeping its position in the processing order (start, end, file position), which the result-selection and response-before-call rules use: a failed inference is not conversation, and its
   ...
   that opens the next request early) is reported, not silently misordered. A child `execute_tool` span that starts before its parent yet ends at the same nanosecond sorts before its call and is reported by the pairing row, likewise. And a consequence of ordering in nanoseconds: a streaming instrumentation that starts and
...
3. **Tool results.** For each `tool_call`, exactly one `tool_result` is *selected*, from two
   ...
   = `gen_ai.tool.call.result` if captured, at the span's end time); else the first `tool_call_response` part, in processing order, in an inference span **other than the emitter and not earlier than it** (the *not-earlier* predicate: a different span whose `startTimeUnixNano` is greater than or equal to the emitter's; a response to a call the same span emits is never legitimate, whatever the clock; the same predicate as the response-before-call row, so the two never disagree; coarse-clock residual: a span with the emitter's start and an earlier end satisfies it, but its result then precedes the call in nanoseconds and the pairing row reports it; only a *zero-duration* span fully coincident with the emitter is undecidable, and there `R` orders the result last (a coincident span of non-zero duration places the response at its start, before the call at the emitter's end, and the pairing row reports it), a stated consequence like the others) whose `gen_ai.input.messages` carries an `id` that matches (`ok: true` meaning *a response was observed*,
...
4. **Ordering.** Events are ordered by a total key: the timestamp in *nanoseconds* (compared
   ...
   keep their order); then the producing span's *processing key*, `(startTimeUnixNano, endTimeUnixNano, file position)`, the same key step 1 processes by, so ordering and processing never disagree (without span end, a zero-duration emitter and a longer presenter sharing a start would be ordered by file position, and a presenter earlier in the file would have its new turn sort before the response it followed); then the
...
| at most one `execute_tool` span per call id | two executions of one call is not a record the adapter can pair |
| the GenAI scope's `schemaUrl`, when present, ends in `/1.37.0`, and two present GenAI scope URLs agree; every producer string that reaches `metadata` is one line, without control characters, at most 1,024 characters | the mapping is versioned by the convention; `metadata` values are bounded v1 strings. A version refusal is reported *together with* every other reachable finding, not alone: the operator sees everything the document would need |
| for every `tool_call_response` part presented in a span S, the selected result's timestamp (tool span end, or presenter start) is no later than `S.startTimeUnixNano` | the trace must not record a result after the conversation it validated shows the model reading it; the tool span wins by preference, and without this row a tool span ending after the presenter would put the assistant's answer before the result it answered |
...
| `service.name`, when present on several resources, is one value | `agent.name` must be a function of the document; differing names are a fault, one finding per resource carrying a name, located by resource index |
| every produced field fits the v1 model: message content ≤ 65,536 characters, `arguments`/`result` ≤ 10,000 nodes and depth 32, `error.type` non-empty and ≤ 256, `error.message` ≤ 1,024, `agent.name`/`tool`/`call_id` identifiers, at most 100,000 events | checked per event *before* the document is built, so a report names the span and part, not a path into a document that was never produced; the final `load_trace` is then a self-check that must pass, and a failure there is a bug: the CLI exits `70`, prints no report, and prints, per validation error, only its `type` and `msg` (never `input`, never `loc`, whose path can name an argument key: agent content); `load_trace` cannot provide that (its `TraceError` carries `loc: msg` strings), so the adapter validates with `Trace.model_validate` and formats `ValidationError.errors()` itself, M2a code untouched, so a bug is never mistaken for a completeness finding and never echoes message text or payloads (a self-check message may name a call id, which is an identifier) |

A report lists each failing check with the index paths (`[d].resourceSpans[i].scopeSpans[j].spans[k]`, then attribute keys, message and part indices) concerned; spans are always named by index path, never by id (locations only, never content: the report is what an operator files, and
message text is the most sensitive thing in the document; resources are named by index, and
the only free-form producer value a report ever carries is a `schemaUrl`, which is a convention URL, not agent content; span ids and, on the self-check path, call ids are identifiers); `d` is the document ordinal (always present, `0` for a single document). The CLI exit code is `1` for a
...
`ledgergate record --from-otel export.json [--out trace.json]` writes the v1 trace (to stdout by default; a write failure on `--out` is exit `2`, "cannot write", and the temporary file is removed; `--out` writes to a uniquely named temporary file in the target's directory (`mkstemp`, prefixed with the target's name), flushes and `fsync`s it, then `os.replace`s it over the target, so neither a process crash nor a second concurrent writer nor an OS crash after the replace leaves a partial *target*; a crash before the replace leaves the uniquely named temporary file, which is not a trace) and exits `0`, or prints the completeness report to stderr and exits `1`; `2` and
