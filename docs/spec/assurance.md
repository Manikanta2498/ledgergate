...
- The level is printed with its reason when it is not the maximum: `L1 (not L2: red-team refund-over-cap failed)`, `L0 (partial selection)`, `L0 (error: 2 scenarios)`. `ledgergate report --conformance CANDIDATE [BASELINE] [--require L1|L2|L3]` prints one line of a fixed grammar, `L<n> (<reasons>)`, where `<reasons>` is a `; `-separated list from a closed vocabulary: `<k> scenarios, <r> red-team` (always first), then, when the level is below L3, exactly the reasons that stopped it: `partial selection`, `nothing scored`, `no red-team`, `error: <k>`, `skipped: <k>`, `correct failed: <ids>`, `red-team failed: <ids>`, `baseline: L<m>` (the baseline's own level, when below L2), `trace changed: <ids>`, `no baseline` (one document, so L3 is undecidable). With one document the level is at most L2; with two it is the *candidate's* level, raised to L3 only when both are L2 and every digest is unchanged. Without `--require` the command exits `0` whatever the level (a rendering); with `--require` it exits `0` when the computed level is at least the required one, `1` below it, `2` on an unreadable document, on `--require L3` with one document, or, with two documents, on any drift precondition failure, so a CI step can require a level in one command. The single-document `md` rendering carries the level line at its top; the drift rendering does not (it renders a `Drift`, not the results), and the two-document conformance verdict is its own output.
...
- **Gated modules**: `src/ledgergate/ledger/` (the deterministic core: a surviving mutant
  ...
  implied one.
- **Invariant**: the set of **unkilled** mutants in the gated modules never grows. "Unkilled" is every mutmut status other than `killed` (and other than caught-by-type-checker, if a type-check command is configured): `survived`, `no tests` (no unit test even executes the function, which is exactly a correctness claim without a test, and the status the gate must see most of all), `timeout`, `suspicious`, `skipped`, `not checked`. The gate compares against a checked-in baseline, `.mutation-baseline.json`, listing every unkilled mutant under its status bucket, and the README number is the total across buckets, `no tests` named separately. `timeout` is CPU-limit based and can flip between runs; a mutant that times out is re-run once before it is judged, and a persistent timeout is keyed like any unkilled mutant. mutmut's names (`<module>.x_<function>__mutmut_<n>`) index mutants positionally within a function, so any edit renumbers the mutants after it; the baseline therefore keys each survivor by `(function, sha256 of the unified diff body)`, the diff mutmut computes for the mutant (as `mutmut show` prints it *below* its `# name: status` header line, which is excluded, since the name carries the positional index and the status changes when the mutant dies); a function-scoped diff survives renumbering; two mutants of one function with identical diffs are one key (they are the same change), and the gate reads mutant statuses from `mutmut results` (the per-mutant `name: status` listing; `export-cicd-stats` gives only counts and cannot drive a ratchet). A run fails if a survivor appears whose key is not in the baseline; a run that kills a baselined mutant *must* remove it in the same change (the gate fails on a stale entry too, so the file cannot drift upward or rot; `make mutation-baseline` regenerates it locally). Configuration uses mutmut 3's current keys (`source_paths`, `only_mutate`, `pytest_add_cli_args_test_selection`, and `also_copy` for `tests/`, `schema/`, `corpus/` and the README, since mutmut runs the tests from a copy under `mutants/` and the unit tests read those paths relative to the working directory; a unit test that spawns a subprocess importing the editable install can never kill a mutant, a known harmless source of `survived` that is baselined, not hidden; the deprecated `paths_to_mutate`/`tests_dir` only warn, in mutmut's own process where pytest's warnings-as-errors does not reach; the current keys are used because they are the documented ones). The baseline starts at whatever M7's first run finds, stated in the commit, and the README reports the count; the honest claim is "does not get worse, and here is the number", not "zero", until the number is zero.
...
- **What is published.** The wheel is BUSL-1.1 (the runtime); `corpus/` and `schema/` are not in it (ADR-0001) and are built and attested in the build job as a tarball, Apache-2.0, then uploaded by the release-asset job as a GitHub release asset alongside the tag, with the provenance bundles of every artefact uploaded as assets too, named `<artefact filename>.intoto.jsonl` (the Scorecard `Signed-Releases` check reads asset names by suffix), so that the offline check in the README, `gh attestation verify <file> --bundle <file>.intoto.jsonl --repo <owner>/ledgergate`, has the bundle as an input and the asset is a verification input, not a filename that satisfies a check. The README's install section says which is which.
