...
| `signed_calls` | Schema 7 ([principals](principals.md)): `principal`, `call_id`, `invocation`, `UNIQUE (principal, call_id)`: every verified envelope's pair, spent on its first presentation whatever the journal answered, written in the invocation's transaction after its row; a `replayed_call` refusal writes none. The `UNIQUE` is the replay guarantee, the `SELECT` its optimisation. |
...
4. **Resolve the key** in `operations` and write the invocation. The invocation's `requested_at` is the transaction's *single* clock reading, taken here and reused as the evaluation time of the approval checks (step 6), of the `PolicyContext`, and (schema 7, [principals](principals.md)) of a signed request's expiry, expiry-bound (`request_expiry_unbounded`) and replay checks, which run here before the key is resolved and record `invalid` on failure; the only other
...
(`invalid`, no outcome), the outbound event; commits; returns). 4. `invocations` (`read`), at the read's single clock reading, where (schema 7) a signed request's expiry, expiry-bound and replay checks run first and record `invalid` on failure exactly as write step 4 does.
