...
**On a failed verdict no evaluating policy code runs** (schema 7: the pure line-lookup `approvers_for` may have run as check 1b and is recomputable from the context's three fields; [principals](principals.md)), not `evaluate`, not subject
...
forms) and the seven of [principals](principals.md): `authentication_malformed`,
...
4. **Resolve the key** in `operations` and write the invocation. The invocation's `requested_at` is the transaction's *single* clock reading, taken here and reused as the evaluation time of the approval checks (step 6), of the `PolicyContext`, and (schema 7, [principals](principals.md)) of a signed request's expiry and expiry-bound (`request_expiry_unbounded`) checks, which run here before the key is resolved and record `invalid` on failure (the replay check is clockless and ran at step 3); the only other
