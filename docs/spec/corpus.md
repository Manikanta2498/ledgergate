...
first `len(before)` invocations (their `tool_call` events) must carry the call ids `setup-1` ... `setup-n`, and their resolutions the setup's attempted digests (the setup steps' fingerprints, which the runner recomputes by
...
scripted_only: true            # required true when window_caps and before interact (see Live)
setup:
