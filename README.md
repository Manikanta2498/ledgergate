...
agent run ──▶ trace (schema v1 | v2) ──▶ invariants + policy ──▶ result.json ──▶ md | junit | sarif
                    ▲
       adapters: OpenTelemetry GenAI (M5) | future conveniences: openai, anthropic, langgraph
...
| M7.5 | Operational readiness: `journal backup`/`restore` under the single-writable-lineage rule, rollover with a predecessor record, `journal status` limits and alerts with WAL checkpointing, crash and load tests, support/upgrade/security-response policies, and the recorded TestPyPI rehearsal ([docs/spec/operations.md](docs/spec/operations.md)) | design draft |
