...
one *transport* principal per session (and, from M8a, any number of signed principals through it; [principals](/spec/principals)). It is a *transport*: everything it does is decode a wire message, hand
...
[--approver NAME=KEYFILE ...] [--token-key-file FILE] [--principal NAME]` (`--approver` replaces schema 6's `--approval-key`; [principals](/spec/principals)).
