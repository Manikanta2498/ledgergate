...
A second schema-7 row, `attributions_are_registered`, walks the `principal_change` and `approver_change` events to compute liveness at every sequence and requires: every resolution whose `authentication` is not `rejected` names a principal live at its sequence with the matching kind, except an `invalid` row with `error_type` `revoked_principal`, whose principal must have a `revoke` before it; every `rejected` row names a live transport principal; every `context.approval.approver`, and the approver named by every `approval_presentation` whose `verified` is true, is live at its sequence; every change event's `by` is a live transport principal at its sequence, except the first event of the document when it is the bootstrap `add` of a transport principal by itself. `no_evidence` for a document without change events.
